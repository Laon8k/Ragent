/*
 * 挑战一：高并发 — ChatQueueLimiter 单元测试
 *
 * 放置路径（运行前请移动到）：
 *   bootstrap/src/test/java/com/nageoffer/ai/ragent/rag/aop/ChatQueueLimiterTest.java
 *
 * 同时将 bootstrap/src/main/resources/lua/queue_claim_atomic.lua
 * 复制到 bootstrap/src/test/resources/lua/queue_claim_atomic.lua
 *
 * 依赖（已在 bootstrap pom 中）：
 *   junit-jupiter, mockito-core, awaitility
 *
 * 测试矩阵：
 *   TC01  限流关闭时请求直通，不入队
 *   TC02  有空闲许可时立即获取并执行
 *   TC03  无可用许可时请求进入 ZSet 排队
 *   TC04  等待超时后 SSE 事件顺序正确（META → REJECT → FINISH → DONE）
 *   TC05  拒绝 SSE 事件发送先于异步落库（优先返回，不被 DB 拖慢）
 *   TC06  Emitter 完成回调触发许可释放与队列通知
 *   TC07  Lua claim 非队头时返回失败，请求留在队列
...
class ChatQueueLimiterTest {
    ...
    void setUp() throws Exception {
        ...
        when(redissonClient.getScoredSortedSet(anyString(), any(StringCodec.class))).thenReturn(zset);
    ...
    void tc01_whenGlobalDisabled_onAcquireCalledDirectly() {
    ...
    void tc07_whenLuaClaimFails_requestRemainsInQueue() {
