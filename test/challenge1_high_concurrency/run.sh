#!/usr/bin/env bash
# 运行挑战一测试并生成详细 HTML 报告
# 用法：bash test/challenge1_high_concurrency/run.sh [ChatQueueLimiterTest|LuaAtomicClaimTest]

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
TEST_CLASS="${1:-ChatQueueLimiterTest}"
REPORT_XML="$ROOT_DIR/bootstrap/target/surefire-reports/TEST-com.nageoffer.ai.ragent.rag.aop.${TEST_CLASS}.xml"
...
.tc-id   { background: #edf2f7; color: #4a5568; font-weight: 700;
           ...
           min-width: 42px; text-align: center; letter-spacing: .03em; }
