#!/usr/bin/env bash
# 运行挑战三测试并生成详细 HTML 报告
# 用法：bash test/challenge3_async_decoupling/run.sh [DelegatingTransactionListenerTest|KnowledgeDocumentChunkTransactionCheckerTest]

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
TEST_CLASS="${1:-DelegatingTransactionListenerTest}"
METADATA_JSON="$SCRIPT_DIR/metadata.json"

# 根据测试类确定包路径和报告路径
case "$TEST_CLASS" in
  DelegatingTransactionListenerTest)
    PKG="com.nageoffer.ai.ragent.framework.mq.producer"
    ;;
  KnowledgeDocumentChunkTransactionCheckerTest)
    PKG="com.nageoffer.ai.ragent.knowledge.mq"
    ;;
  *)
