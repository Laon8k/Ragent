#!/usr/bin/env bash
# 运行挑战二测试并生成详细 HTML 报告
# 用法：bash test/challenge2_high_availability/run.sh [ModelCircuitBreakerTest|ProbeStreamBridgeTest]

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
TEST_CLASS="${1:-ModelCircuitBreakerTest}"
METADATA_JSON="$SCRIPT_DIR/metadata.json"

# 根据测试类确定包路径和报告路径
case "$TEST_CLASS" in
  ModelCircuitBreakerTest)
    PKG="com.nageoffer.ai.ragent.infra.model"
    ;;
  ProbeStreamBridgeTest)
    PKG="com.nageoffer.ai.ragent.infra.chat"
    ;;
  *)
...
REPORT_XML="$ROOT_DIR/infra-ai/target/surefire-reports/TEST-${PKG}.${TEST_CLASS}.xml"
REPORT_HTML="$ROOT_DIR/infra-ai/target/surefire-reports/${TEST_CLASS}-report.html"
...
tc_map = {tc.attrib["name"]: tc for tc in root.findall("testcase")}
